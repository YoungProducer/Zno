...
export const testSetUpReducer = (state = testSetUpInitialState, action) => {
  switch (action.type) {
    ...
    case SET_UP_TASKS: {
      const { subject, subSubject, deepType, theme, testName } = action.payload;

      return {
        ...state,
        selectedSubject:
          subject[0] + subject.slice(1, subject.length).toLowerCase(),
...
export const givedAnswersReducer = (
  state = givedAnswersInitialState,
  action
) => {
  switch (action.type) {
    case INIT_ANSWERS: {
    ...
    case GIVE_AN_RELATION_ANSWER: {
      state.givedAnswers[action.payload.testId][action.payload.index] =
    ...
    case NULLIFY_ANSWER: {
      ...
      if (action.payload.type === 2) {
        ...
        for (
          let i = 0;
          i < state.givedAnswers[action.payload.testId].length;
          i++
        ) {
          fields.push("");
        }
...
export const selectedAnswersReducer = (
  state = selectedAnswersInitialState,
  action
) => {
